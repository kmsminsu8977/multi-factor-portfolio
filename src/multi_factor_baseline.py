...
PROJECT = {"analysis_type": "multi_factor", "category": "Factor Investing", "columns": ["asset", "value_score", "momentum_score", "quality_score", "low_volatility_score", "market_cap"], "methods": ["팩터별 원점수를 z-score로 바꾸고 동일 가중 composite score를 계산한다.", "상위 종목은 composite score에 비례해 배분하되 concentration을 확인한다.", "종목명과 점수는 팩터 결합 절차를 설명하는 합성 예시다."], "module_name": "multi_factor_baseline", "objective": "가치, 모멘텀, 퀄리티, 저변동성 점수를 합성해 포트폴리오 후보를 선별한다.", "question": "복수 팩터를 같은 척도로 표준화하면 어떤 종목이 안정적으로 상위권에 남는가?", "rows": [["ALPHA_BANK", 0.78, 0.42, 0.69, 0.55, 18.2], ["BETA_CHIP", 0.35, 0.82, 0.74, 0.48, 44.5], ["CALM_HEALTH", 0.62, 0.51, 0.81, 0.83, 22.1], ["DELTA_RETAIL", 0.57, 0.66, 0.45, 0.52, 9.4], ["ECO_GRID", 0.49, 0.73, 0.68, 0.61, 13.8]], "sample_name": "factor_scores.csv", "title_en": "Multi-Factor Portfolio", "title_ko": "멀티팩터 포트폴리오"}
